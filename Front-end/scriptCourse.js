...
fetch('http://10.2.130.45:9090/availablecourses', {
  method: 'GET',
  headers: {
    'token': token
  }
...
const showPopup = (data) => {
    ...
    <div id="modal">
      <div id="content">
        ...
        <h1 id="reserve-seat-title">Title: ${data.Title}</h1>
        <p id="lector-p">Lector: ${data.Lector}</p>
    ...
    for(let i = 1; i <= 15; i++){
        ...
        `
            ...
            document.getElementById('seat-container').innerHTML += `
            <button class="seat-btn seat-btn-${i}" onclick="confirm(${data.ID},${i})">Seat ${i}</button>
        `
        }
        //
    }
